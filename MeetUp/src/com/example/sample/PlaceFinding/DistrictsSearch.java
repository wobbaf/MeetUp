...
private void init(Initiator initiator, List<Invited> allInvited)
{
    ...
    public Stack<String> findClosestPlaceInBestDistrict(Stack<Integer> bestDistrictID, long [] timeTravelForInitiator) {
       ...
       // System.out.println(placesId.toString());
        ...
        SortedMap<Double,String> meanToPlaceId = new TreeMap<>();
        ...
        List<String>distinctFavourite =new ArrayList<String> (allDifferent);
            ...
            for (int j = 0; j < distinctFavourite.size(); j++) {
                ...
                meanToPlaceId.put(mean,distinctFavourite.get(j));
        ...
        List<Double> keyList = new ArrayList<Double>(meanToPlaceId.keySet());
        for(int i =meanToPlaceId.size()-1; i>=0; i--)
        {
            bestPlaces.add(meanToPlaceId.get(keyList.get(i)));
        }
        meanToPlaceId.clear();
        ...
        if (placesId.size() > 0) {
            for (int j = 0; j < commonFavourite.size(); j++) {
                ...
                meanToPlaceId.put(mean,commonFavourite.get(j));
            }
        }

        keyList = new ArrayList<Double>(meanToPlaceId.keySet());
        for(int i =meanToPlaceId.size()-1; i>=0; i--)
        {
            bestPlaces.add(meanToPlaceId.get(keyList.get(i)));
        }
