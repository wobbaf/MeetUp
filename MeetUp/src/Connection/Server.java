...
public class Server extends Agent{
								...
								case "4":
									if (r.state != null && !done.contains(rec.getSender().getLocalName().toString())) {
										...
										if (((agreedInvitedUsers.size() == ctr) && gbad >= 0) || System.currentTimeMillis() - startTime >= 180000) {
											...
											agreedInvitedUsers = new ArrayList<>();
											allbestPlacesForThem.clear();
											initiator = null; 
	...
	private String clearContent(){
		...
		parser.XMLParse p = new parser.XMLParse();
		//p.instance();
			p.instance().type = null;
			p.instance().id = null;
			p.instance().location = null;
			p.instance().state = null;
			p.instance().time = null;
			p.instance().placeType = null;
			p.instance().placeId = null;
			p.instance().friends = null;
			p.instance().favPlaces = null;
		content = p.Parse();
		return content;
	}
